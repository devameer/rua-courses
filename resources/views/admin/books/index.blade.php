        ...
        <div class="card">
            ...
            <div class="card-header border-0 pt-5">
                ...
                <div class="card-toolbar">
                    <a href="{{ route('admin.books.create') }}" class="btn btn-sm btn-light-primary">
                            ...
                            @foreach($books as $book)
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="symbol symbol-50px me-5">
                                                <img src="{{ asset($book->image) }}" class="" alt=""/>
                                            </div>

                                            <div class="d-flex justify-content-start flex-column">
                                                <a href="#" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $book->name }}</a>
                                                {{-- <span class="text-muted fw-bold text-muted d-block fs-7">{{ $row['product']['specs'] }}</span> --}}
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ date('M d, Y' , strtotime($book->created_at)) }}</a>
                                    ...
                                    <td class="text-end">
                                        ...
                                        <a href="{{ route('admin.books.edit', $book) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                            {!! theme()->getSvgIcon("icons/duotone/Communication/Write.svg", "svg-icon-3") !!}
                                        </a>

                                        <a href="{{ route('admin.books.delete', $book) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
